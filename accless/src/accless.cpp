...
static bool validHardwareAttestation(std::string &jwtStrOut) {
    ...
    std::string jwtStr = accless::attestation::snp::getAttestationJwt(
        "", "", "gid", "wid", "nid");
#endif
...
bool checkChain(const std::string &workflow, const std::string &function,
    ...
    -----------------------------------------------------------------------
        ...
        if (!validHardwareAttestation(jwtStr)) {
            std::cout << "accless: error validating hw attestation" <<
    std::endl; return false;
        }

        std::string teeIdentity = accless::jwt::getProperty(jwtStr,
    "tee_identity");
    ...
    -----------------------------------------------------------------------
        ...
        std::vector<uint8_t> ctCtxTrimmed(ctCtx.begin() + AES256CM_NONCE_SIZE,
                                          ctCtx.end());
        // auto ptCtx = accless::aes256gcm::decrypt(teeSymKey, nonceCtx,
    ctCtxTrimmed);
        ...
        std::vector<uint8_t> nonceCertChain(
            ctAesCertChain.begin(), ctAesCertChain.begin() +
    AES256CM_NONCE_SIZE); std::vector<uint8_t> ctCertChain(
        ...
        // TODO: this does not work well for functions with more than one
    parent! auto expectedChain = accless::dag::getCallChain(dag, function);
        ...
        // Now use our attributes to decrypt the actual contents of the cert
    chain auto certChain = ctx.cpAbeDecrypt(attributes, ptAesCertChain); if
    (certChain.empty()) { std::cerr << "accless: error decrypting certificate
    chain" << std::endl; return false;
        }
    ...
    int32_t chain(const std::string &workflow, const std::string
    &parentFuncName, int parentIdx, const std::string &funcName, int idx, const
    std::string &inputData) { if (!on()) { #ifdef __faasm return
    faasmChainNamed(funcName.c_str(), (uint8_t *)inputData.c_str(),
                                   inputData.size());
    #else
            return 0;
    #endif
