...
pub unsafe extern "C" fn verify_jwt(jwt_cstr: *const c_char) -> bool {
    let mut x5c_certs: Vec<&str> = generated_x5c_certs::INJECTED_X5C_CERTS.to_vec();
    x5c_certs.push(TEST_CERT);
...
mod tests {
    ...
    const TEST_CERTS: [&str; 1] = [TEST_CERT];
