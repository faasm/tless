...
std::string getMockSgxAttestationJwt() {
    ...
    std::string body = utils::buildRequestBody(quoteB64, runtimeB64, MOCK_GID, MOCK_WORKFLOW_ID, MOCK_NODE_ID);

    std::string response = accless::attestation::getJwtFromReport("/verify-sgx-report", body);
