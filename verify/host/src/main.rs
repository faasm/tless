...
use methods::{
    EDAG_VERIFY_ELF, EDAG_VERIFY_ID
};
...
fn load_dag(yaml_path: &str) -> DagGraph {
    ...
    file.read_to_string(&mut contents).expect("tlessctl(verify): faild to read yaml");
    let dag: DagGraph = serde_yaml::from_str(&contents).expect("tlessctl(verify): failed to parse yaml");
...
fn verify_edag(
    ...
    let input = (dag, cert_chain, verifying_key.to_encoded_point(true), num_chains, skip_verify);
...
fn main() {
    ...
    let funcs_json = to_string(&funcs).expect("tlessctl(verify): failed to serialize nodes to json");
    ...
    // leafs in the eDag
    let receipt = verify_edag(&cert_chain, &dag, signing_key.verifying_key(), num_chains, skip_verify);
    ...
    println!("Signature valid: {} - DAG preserved: {}", result.signature_valid, result.dag_preserved);
}
