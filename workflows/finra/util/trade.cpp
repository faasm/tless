...
std::vector<uint8_t> serializeTradeVector(const std::vector<TradeData>& trades)
{
    ...
    for (const auto& trade : trades) {
        ...
        buffer.insert(buffer.end(), serializedTrade.begin(), serializedTrade.end());
    }
    return buffer;
}
...
std::vector<TradeData> deserializeTradeVector(const std::vector<uint8_t>& buffer)
{
    ...
    for (size_t i = 0; i < buffer.size(); i += tradeSize) {
        ...
        std::vector<uint8_t> tradeBuffer(buffer.begin() + i, buffer.begin() + i + tradeSize);
...
std::vector<uint8_t> serializePortfolio(const Portfolio& portfolio)
{
    ...
    size_t bufferSize = sizeof(numHoldings) + (numHoldings * sizeof(PortfolioHolding));
    ...
    std::memcpy(buffer.data() + offset, portfolio.holdings.data(), numHoldings * sizeof(PortfolioHolding));
...
Portfolio deserializePortfolio(const std::vector<uint8_t>& buffer)
{
    ...
    std::memcpy(portfolio.holdings.data(), buffer.data() + offset, numHoldings * sizeof(PortfolioHolding));
        ...
        for (size_t i = 1; i < trades.size(); ++i) {
            ...
            if (trades[i].ticker == holding.ticker
              && trades[i - 1].ticker == holding.ticker) {
                // Detect volume spike
                double volumeChange =
                  static_cast<double>(trades[i].volume) / trades[i - 1].volume;
                // Detect significant price change
                double priceChange =
                  std::fabs(trades[i].close - trades[i - 1].close) / trades[i - 1].close;
                ...
                if (volumeChange > volumeSpikeThreshold
                  && priceChange > priceChangeThreshold) {
                    std::cout << "finra(audit): potential insider trade detected"
                              << std::endl
                              << "finra(audit): for "
                              << holding.ticker
                              << " on date "
