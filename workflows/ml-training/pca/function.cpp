...
void loadImages(const std::string &us, const std::string &bucketName,
    ...
    if (ret != 0) {
        printf("ml-training(%s): error: error getting bytes from key: %s "
               "(bucket: %s)\n",
               us.c_str(), s3file.c_str(), bucketName.c_str());
    }
    ...
    while (std::getline(ss, image, ',')) {
        if ((label % progressEvery) == 0) {
            std::cout << "ml-training(" << us << "): loaded " << label << "/"
        ...
        if (ret != 0) {
            printf("ml-training(%s): error: error getting bytes from key: %s "
                   "(bucket: %s)\n",
                   us.c_str(), image.c_str(), bucketName.c_str());
        }
...
std::vector<std::vector<uint8_t>> splitAndSerialize(const cv::Mat &mat,
                                                    int numMats) {
...
int main(int argc, char **argv) {
    ...
    if (parts.size() != 3) {
        std::cerr << "ml-training(pca): error parsing partition input"
                  << std::endl;
        return 1;
    }
    ...
    std::cout << "ml-training(" << us << "): running PCA on S3 dir " << s3dir
              << std::endl
              << "ml-training(" << us << "): chaining into " << numTrainFuncs
              << " parallel random forest trees" << std::endl;

    // Load images
    std::cout << "ml-training(" << us << "): beginning to load images..."
              << std::endl;
    std::vector<cv::Mat> images;
    std::vector<int> labels;
    loadImages(us, bucketName, s3dir, images, labels);
    std::cout << "ml-training(" << us << "): " << images.size()
    ...
    std::cout << "ml-training(" << us << "): performing PCA analysis..."
              << std::endl;
    ...
    std::cout << "ml-training(" << us << "): PCA on images succeded!"
              << std::endl;

    // Split and invoke training
    std::cout << "ml-training(" << us
              << "): splitting and serializing results..." << std::endl;
    ...
    std::cout << "ml-training(" << us << "): splitting and serializing done!"
              << std::endl;

    // Upload the serialized results
    for (int i = 0; i < serializedMats.size(); i++) {
        std::string dataKey =
            "ml-training/outputs/" + us + "/rf-" + std::to_string(i) + "-data";
#ifdef __faasm
        // Overwrite the results
        int ret = __faasm_s3_add_key_bytes(bucketName.c_str(), dataKey.c_str(),
                                           serializedMats.at(i).data(),
                                           serializedMats.at(i).size(), true);
        if (ret != 0) {
            std::cerr << "ml-training(" << us
                      << "): error uploading PCA data for training"
                      << std::endl;
            return 1;
        }
#else
        ...
        // Upload the labels
        std::string labelsKey = "ml-training/outputs/" + us + "/rf-" +
                                std::to_string(i) + "-labels";
#ifdef __faasm
        // Overwrite the results
        ret = __faasm_s3_add_key_bytes(bucketName.c_str(), labelsKey.c_str(),
                                       serializedLabels.at(i).data(),
                                       serializedLabels.at(i).size(), true);
        if (ret != 0) {
            std::cerr << "ml-training(" << us
                      << "): error uploading labels data for training"
                      << std::endl;
            return 1;
        }
#else
    ...
    for (int i = 0; i < numTrainFuncs; i++) {
        ...
        std::string labelsKey = "ml-training/outputs/" + us + "/rf-" +
                                std::to_string(i) + "-labels";
        std::string pcaInput = std::to_string(id) + ":" + std::to_string(i) +
                               ":" + dataKey + ":" + labelsKey;
#ifdef __faasm
        // int pcaId = faasmChainNamed("rf", (uint8_t*) pcaInput.c_str(),
        // pcaInput.size());
