...
std::map<std::string, int> wordCount = {
    {"JavaScript", 0},  {"Java", 0},    {"PHP", 0},   {"Python", 0},
    {"C#", 0},          {"C++", 0},     {"Ruby", 0},  {"CSS", 0},
    {"Objective-C", 0}, {"Perl", 0},    {"Scala", 0}, {"Haskell", 0},
    {"MATLAB", 0},      {"Clojure", 0}, {"Groovy", 0}};
...
int main(int argc, char **argv) {
    ...
    if (parts.size() != 2) {
        std::cerr << "word-count(mapper): error parsing driver input"
                  << std::endl;
        return 1;
    }
    ...
    if (!accless::checkChain("word-count", "mapper", id)) {
        std::cerr << "word-count(" << us << "): error checking TLess chain"
                  << std::endl;
        return 1;
    }
    ...
    int ret = __faasm_s3_get_key_bytes(bucketName.c_str(), s3ObjectKey.c_str(),
                                       &keyBytes, &keyBytesLen);
    if (ret != 0) {
        std::cerr << "word-count(" << us
                  << "): error getting key bytes from key: " << s3ObjectKey
    ...
    std::cout << "word-count(" << us << "): writting result to " << resultsKey
              << std::endl;
#ifdef __faasm
    // Overwrite the results key
    ret = __faasm_s3_add_key_bytes(bucketName.c_str(), resultsKey.c_str(),
                                   (void *)thisWordCount.c_str(),
                                   thisWordCount.size(), true);
#else
