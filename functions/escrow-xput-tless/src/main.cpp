...
void tpmRenewAkCert() {
  ...
  if (renewalRequired) {
    ...
    if (replaceResult.code_ != AttestationResult::ErrorCode::SUCCESS) {
      std::cerr << "accless: failed to renew AkCert: " << result.description_
                << std::endl;
...
std::string maaGetJwtFromParams(AttestationClient *attestationClient,
  ...
  if (attResult.code_ != attest::AttestationResult::ErrorCode::SUCCESS) {
    std::cerr << "accless: error getting attestation from attestation client"
              << std::endl;
    Uninitialize();
...
std::chrono::duration<double> run_requests(int numRequests,
                                           int maxParallelism) {
  // ---------------------- Set Up CP-ABE --------------------------------------
