...
/// # Description
///
/// This struct corresponds to the request that SGX-Faasm sends to verify an SGX report. Some of
/// the fields are legacy from when we used Azure's Attestation Service, and they are now unused.
/// The implementation of the client-side protocol is upstreamed in Faasm, because it relies on
/// coordination from inside/outside the Faaslet (i.e. WASM module) and inside/outside the SGX
/// enclave. We provide a mock implementation of the SGX client with Accless' attestation library
...
pub struct SgxRequest {
    ...
    quote: String,
    /// Additional base64-encoded data that we send with the enclave as part of the enclave held
    /// data. Even if slightly redundant, it is easier to access as a standalone field, and we
    /// check its integrity from the quote itself, which is signed by the QE.
